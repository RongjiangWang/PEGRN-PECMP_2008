      subroutine pecdisc(ns,npsum,tmax)
      implicit none
c
...
c     =================
c~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      integer is,izs,ix,iy,nx,ny,nxy,ips,iptch
      double precision dr,x,y,x0,y0,dx,dy,dzs,st,di,ra
      ...
      double precision sm(3,3),om(3,3)
c
      double precision PI
...
c
     ...
     &       zref(is)+(width(is)-1.5d0*dy)*ssd.gt.zs2)then
            nwarn=nwarn+1
            print *,'Warning in pecdisc: the fault plane in the'
...
c
              px(ips,izs)=90.d0-smc/DEG2RAD
              py(ips,izs)=dmod(yref(is)+bga/DEG2RAD,360.d0)
              pz(ips,izs)=pz0
              isno(ips,izs)=is
c
...
100     continue
      enddo
      write(*,*)'------------------------------------------------'
      write(*,'(a,i7)')' total number of point sources: ',npsum
c
      return
      end
